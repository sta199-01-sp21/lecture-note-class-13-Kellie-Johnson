...
```{r}
library(usethis)
use_git_config(user.name = "Kellie-Johnson", user.email = "kaj56@duke.edu")
```
...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-Kellie-Johnson/manhattan.csv")
```
...
This means that we are 95% confident that the mean price of a 1 bedroom 
apartment in Manhattan is between 2299 and 2977 dollars. 
...
This means that I am 95% confident that the median price of 1 bedroom apartments
in Manhattan is between 2162 and 2875 dollars. 
...
**Interpretation**: This means that we are 95% confident that the proportion of 
mice that died while not on the treatment is between 69% and 79.7%
...
```{r practice_2_ci}
ci_mice_lived <- get_ci(boot_prop_lived, level = .90)
ci_mice_lived
```
...
I am 90% confident that the proportion of mice who lived while on the treatment
regardless of dose is between 55.2% and 65.1%
